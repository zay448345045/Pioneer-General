...
import com.udawos.pioneer.ui.AboutButton;
import com.udawos.pioneer.ui.Archs;
import com.udawos.pioneer.ui.ExitButton;
import com.udawos.pioneer.ui.PrefsButton;
...
public class TitleScene extends PixelScene {
	...
	public void create() {
		...
		float height = title.height + 
			(Pioneer.landscape() ? DashboardItem.SIZE : DashboardItem.SIZE * 2);
		...
		//		Pioneer.switchNoFade( BadgesScene.class );
		//	}
		//};
		//add( btnBadges );
		...
		DashboardItem btnPlay = new DashboardItem( TXT_PLAY, 0 ) {
			@Override
			protected void onClick() {
				Pioneer.switchNoFade( StartScene.class );
			}
		};
		add( btnPlay );
		...
		//	@Override
		//	protected void onClick() {
		//		Pioneer.switchNoFade( RankingsScene.class );
		//	}
		//};
		//add( btnHighscores );
		...
		if (Pioneer.landscape()) {
			...
			btnPlay		.setPos( w / 2, y );
			//btnPlay			.setPos( btnHighscores.left() - btnPlay.width(), y );
		} else {
            btnPlay.setPos( w / 3, (h + height) / 2 - DashboardItem.SIZE );
        ...
        btnAbout.setPos( 0, h- btnAbout.height());
        add (btnAbout);
